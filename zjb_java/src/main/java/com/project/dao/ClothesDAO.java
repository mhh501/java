...
public class ClothesDAO {
    ...
    public boolean add(Clothes c) {
        String sql = "INSERT INTO clothes (name, style, category, size, price, stock, image, description) VALUES (?,?,?,?,?,?,?,?)";
        try (Connection conn = DBHelper.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public boolean update(Clothes c) {
        String sql = "UPDATE clothes SET name=?, style=?, category=?, size=?, price=?, stock=?, image=?, description=? WHERE id=?";
        try (Connection conn = DBHelper.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, c.getName());
            ps.setString(2, c.getStyle());
